    ...
    public class ExpressionSerializer
    {
        ...
        private XElement GenerateXmlFromExpressionCore(Expression e)
        {
            ...
            foreach (var prop in properties)
            {
                try
                {
                    ...
                    {
                        MemberExpression memberExpression = val as MemberExpression;
        ...
        private object ParseConstantFromAttribute(XElement xml, string attrName, Type type)
        {
            ...
            if (typeof(string[]).Equals(type))
                return Convert.ChangeType(objectStringValue.Split(','), type, CultureInfo.CurrentCulture);
        ...
        private object ParseConstantFromElement(XElement xml, string elemName, Type type)
        {
            ...
            //*******************************
            if (!string.IsNullOrEmpty(objectStringValue) && type.ToString().EndsWith("[]"))
            {
                System.Xml.Serialization.XmlSerializer dser = new System.Xml.Serialization.XmlSerializer(type);
                StreamReader mem2 = new StreamReader(new MemoryStream(System.Text.Encoding.UTF8.GetBytes(objectStringValue)),
                                     System.Text.Encoding.UTF8);
                try
                {
                    object obj = dser.Deserialize(mem2);
                    mem2.Close();
                    return Convert.ChangeType(obj, type, CultureInfo.CurrentCulture);
                }
                catch (Exception)
                {
